...
MODEL_CLASSES = {"bert": (BertConfig, DMBERT, BertTokenizer)}


def calculate_scores(preds, labels, dimE):
    positive_labels = list(range(1, len(dimE)))   # assume 0 is NA
    ...
    info = classification_report(labels, preds,
                                 labels=positive_labels,
                                 digits=4,
                                 output_dict=True,
                                 target_names=dimE[1:])
    with open('./performance/dmbert-prf_report.txt', 'w', encoding='utf-8') as f:
        json.dump(info, f, indent=4, ensure_ascii=False)

    conf_matrix = confusion_matrix(labels, preds, labels=list(range(0, len(dimE))))
    np.save('./performance/confusion_matrix.npy', conf_matrix)
...
def train(args, train_dataset, model, tokenizer):
    ...
    set_seed(args)  # Added here for reproduce performance
    ...
    for _ in train_iterator:
        ...
        for step, batch in enumerate(epoch_iterator):
            ...
            inputs = {
                ...
                "token_type_ids": batch[2] if args.model_type in ["bert", "xlnet"] else None,  # XLM don't use
                "maskL": batch[3],
                "maskR": batch[4],
                "labels": batch[5],
            }
                    ...
                    if (
                        ...
                        if results["eval_f1"] > best_dev_f1:
                            ...
                            if best_steps:
                                ...
                                logger.info("best precision: {}, recall: {}, f1: {}".format(results['eval_p'], results['eval_recall'], results['eval_f1']))
                                with open('./saved/best_step_info.txt', 'w', encoding='utf-8') as f:
                                    json.dump({'best_checkpoint': best_steps, 'f1': results['eval_f1'], 'precision': results['eval_p'], 'recall': results['eval_recall']}, f, indent=4, ensure_ascii=False)
                            if args.do_test:
...
def evaluate(args, model, tokenizer, prefix="", test=False, infer=False):
...
def load_and_cache_examples(args, task, tokenizer, evaluate=False, test=False):
    ...
    cached_features_file = os.path.join(args.data_dir,
                                        "cached_{}_{}_{}_{}".format(cached_mode,
                                                                    list(
                                                                        filter(None, args.model_name_or_path.split("/"))
                                                                    ).pop(),
    ...
    if os.path.exists(cached_features_file) and not args.overwrite_cache:
        ...
        if evaluate:
            ...
            logger.info("Valid number: %s", str(len(examples)))
        elif test:
            examples = processor.get_test_examples(args.data_dir)
            logger.info("Test number: %s", str(len(examples)))
        else:
...
def main():
    ...
    config.update({'input_max_seq_length': args.max_seq_length})  # self-added
    ...
    model = model_class.from_pretrained(args.model_name_or_path,
                                        ...
                                        cache_dir=args.cache_dir if args.cache_dir else None)
    if args.local_rank == 0:
        torch.distributed.barrier()  # Make sure only the first process in distributed training downloads model&vocab
